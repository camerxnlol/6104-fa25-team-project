...
## Design Summary
### Backend
On the backend, the main changes in our project were in the CountryRecommendation concept, specifically generating the song recommendations. This concept went through many stages in the course of our development due to us learning that certain things just couldnʻt work as they were. For example, we learned that it isnʻt possible for an LLM to return good links to youtube videos, nor correctly find songs that belonged to an artist. It will always hallucinate youtube links and return songs that belonged to another artist, or simply doesn't exist. We went through a lot of approaches to fix these issues. In the end, we landed on an approach that used an LLM --> Youtube API --> Spotify API --> LLM pipeline to get artists, find songs, and validate them. This was able to yield good results, as all of our system generated recommendations were able to be both accurate and exist. Another small change we made to CountryRecommendations was making genre a required field like the other fields. This made things much easier and simpler for us.
...
I think that this project was a good experience. It was an opportunity to put into practice the skills that I learned over the semester and from my personal project. I think that an especially helpful skill was using AI agents like Context and Copilot to help me code/design code. Since I did it in my personal project, I was able to get the hang of it (figuring out what did and didnʻt work) and use it effectively in this project.

Doing this project with a team added a difficult, but important, challenge. Coordinating each others code and responsibilities was tough at times, but I think it was a good way to ensure that the core concepts we are meant to take away from this class got across to us well. I felt particularly proud when we were each able to work on our own concepts in the backend seperately and push them all together without any merge errors or other dependency errors. Being able to design and code concepts that were modular and successful is a good application of the lessons we learned this semester.

### Xuan

**What I learned**

LLMs still have a lot of weaknesses in certain areas. I was not aware that LLMs could not reliably find songs belonging to an artist. At first, I tried prompt engineering to find a way to have it return better results. However, I took a step back and asked the LLM whether this was possible to have it generate matching songs at all, and learned that LLM would struggle to fact check whether a song belonged to an artist without access to the internet. This led me to add in the Spotify API, since it already has the data linking artists and songs, and switch gears to use LLM only for the artist suggestion and final validation, which LLMs are good at. It was a good learning experience to learn to step back and ask whether the current solution is a valid solution and pivot to using more reliable data sources.

**Working on a team + communication**

Our team did a great job of communicating with each other and working on tasks in parallel. We had minimal merge conflicts and code changes were compatible with each other. I've realized how important it was to let the team know which part you are working on to prevent duplicate work being done and incompatibility issues. Furthermore, each team member has their own coding strengths and field of expertise, and we utilized that by having team members work on parts that tailored to their strengths.