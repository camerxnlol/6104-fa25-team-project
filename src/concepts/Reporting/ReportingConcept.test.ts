...
Deno.test("Reporting Concept: User Journeys", async (t) => {
  await t.step(
    ...
    async () => {
      ...
      assertEquals(
        reportersResult[0]?.reporters,
        [],
        "Initial reporters should be empty.",
      );
  ...
  await t.step(
    ...
    async () => {
      ...
      assertEquals(
        countResult[0]?.count,
        0,
        "Count should be 0 after unreport.",
      );
      reportersResult = await reporting._getReporters({ objectId });
  ...
  await t.step(
    ...
    async () => {
      ...
      console.log(
        `2. User '${userId}' reports '${objectId}' (should succeed).`,
      );
      await reporting.Report({ objectId, userId });

      console.log(
        `3. User '${userId}' reports '${objectId}' again (should fail).`,
      );
      const report2 = await reporting.Report({ objectId, userId });
      assertExists(
        (report2 as { error: string }).error,
      ...
      console.log(
        `4. User '${userId}' unreports '${objectId}' (should succeed).`,
      );
      await reporting.Unreport({ objectId, userId });

      console.log(
        `5. User '${userId}' unreports '${objectId}' again (should fail).`,
      );
      const unreport2 = await reporting.Unreport({ objectId, userId });
      assertExists(
        (unreport2 as { error: string }).error,
        "Second unreport should fail.",
      );
  ...
  await t.step(
    ...
    async () => {
      ...
      assertEquals(
        reportersResult[0]?.reporters,
        [reportingUser],
        "Reporters must be unchanged.",
      );
      console.log("State is unchanged, as expected.");

      console.log(
        "Success: The system correctly prevented an unauthorized unreport.",
      );
      await client.close();
    },
  );
